...
- Create AWS IAM user Harikha_manthena and create access key.
- Configure AWS CLI and provide access key, it will be configured.
- Create AWS S3 bucket.
- Write a python code that takes data from local and puts the file to S3 bucket (provide s3 path here and run the code in VS studio).
- CSV files are loaded to S3 bucket.
- Each file represents a different subject area and grain (store master, store+dept sales, store+date economics).
...
- Using python code, I uploaded all CSV files to S3 under a folder like raw_data/.
- S3 keeps raw files “as-is” for audit, durability, and reprocessing.
- Create IAM policy (snowflake_access9) for accessing Snowflake where I configured the bucket and folder name.
- Create IAM role.
- Successfully setup done of S3 with Snowflake using External Stage.
